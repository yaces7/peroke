...
function elementKartiOlustur(element) {
    ...
    kart.className = 'kart';
    kart.dataset.atomNo = element.atomNumarasi || element.atom_no;
    
    // Element türüne göre kart rengi belirle
    const grupTuru = element.grupTuru || element.element_turu;
    const renk = elementTuruneGoreRenkGetir(grupTuru);
    kart.style.backgroundColor = renk;
    
    // Kart içeriğini oluştur
    const sembol = element.sembol;
    const isim = element.isim;
    const atomNo = element.atomNumarasi || element.atom_no;
    const grup = element.grupNumarasi || element.grup;
    const periyot = element.periyotNumarasi || element.periyot;
    
    kart.innerHTML = `
        <div class="kart-baslik">${sembol}</div>
        <div class="kart-atomno">${atomNo}</div>
        <div class="kart-isim">${isim}</div>
        <div class="kart-detay">Grup: ${grup}, Periyot: ${periyot}</div>
    `;
    
    return kart;
}

// Element türüne göre renk belirleme
function elementTuruneGoreRenkGetir(grupTuru) {
    const renkler = {
        'Alkali Metal': '#ff6666',
        'Toprak Alkali Metal': '#ffdaac',
        'Geçiş Metali': '#ffc0c0',
        'Metal': '#cccccc',
        'Yarı Metal': '#cccc99',
        'Ametal': '#a0ffa0',
    ...
    return renkler[grupTuru] || '#f0f0f0';
}

// Global erişim için
window.elementKartiOlustur = elementKartiOlustur;
